        ...
        {chapters.map((ch, i) => (
          <div key={i} className="chapter-card">
            <div className="chapter-card-head">
              <h3>{ch}</h3>
              <p className="muted">High-value revision & PYQ videos</p>
            </div>

            <div className="videos-preview">
              {(videos.physics[ch] || []).slice(0,3).map((v, idx) => (
            ...
            <div className="chapter-actions">
              <a className="btn small" href={(videos.physics[ch] && videos.physics[ch][0]) ? videos.physics[ch][0].url : "#"} target="_blank" rel="noopener noreferrer">Open playlist</a>
            </div>
          </div>
        ))}
      </div>
